...
from utils import *
from data_loader.utils import load_file_list, read_frame, refine_image, refine_image_pad
...
def eval_quan_qual(config):
    ...
    for i, frame_name in enumerate(input_c_file_path_list):
        ...
        # Read image
        C = read_frame(input_c_file_path_list[i], config.norm_val, rotate)
        _, ori_h, ori_w, _ = C.shape
        C = refine_image_pad(C, refine_val)
        C = torch.FloatTensor(C.transpose(0, 3, 1, 2).copy()).to(config.device)

        if input_l_file_path_list is not None:
            L = refine_image_pad(read_frame(input_l_file_path_list[i], config.norm_val, rotate), refine_val)
            L = torch.FloatTensor(L.transpose(0, 3, 1, 2).copy()).to(config.device)

        if input_r_file_path_list is not None:
            R = refine_image_pad(read_frame(input_r_file_path_list[i], config.norm_val, rotate), refine_val)
            R = torch.FloatTensor(R.transpose(0, 3, 1, 2).copy()).to(config.device)

        if gt_file_path_list is not None:
            GT = refine_image_pad(read_frame(gt_file_path_list[i], config.norm_val, rotate), refine_val)
