...
TOOL_DESCRIPTIONS = {
    "hat_gan": "Model Super-Resolution (HAT-GAN) chuyên tăng độ nét và chi tiết.",
    "swinir_gan": "Model Super-Resolution (SwinIR) cân bằng giữa khử nhiễu và làm nét.",
    "real_esrgan": "Model Super-Resolution phổ biến, mạnh về phục hồi ảnh đời thực.",
    "restormer": "Chuyên gia khử nhiễu (Denoising) và khử mờ do chuyển động.",
    "maxim": "Model đa năng của Google (Denoising, Deblurring, Deraining).",
    "gfpgan": "Phục hồi khuôn mặt (Face Restoration) bằng GAN.",
    "codeformer": "Phục hồi khuôn mặt SOTA, xử lý tốt ảnh mặt bị biến dạng nặng.",
    "scunet": "Khử nhiễu chuyên sâu (Denoising).",
    "nafnet": "Khử mờ (Deblurring) hiệu quả cao.",
    "hpsv2": "Mô hình đánh giá thẩm mỹ con người (Human Preference Score).",
    "musiq": "Mô hình đánh giá chất lượng ảnh đa tỉ lệ.",
    "niqe": "Chỉ số đánh giá chất lượng tự nhiên (không cần ảnh gốc).",
    "topiq_nr": "Mô hình đánh giá chất lượng dựa trên Transformer."
}

def get_tool_desc(tool_name):
    clean_name = tool_name.replace("tool-", "").lower()
    return TOOL_DESCRIPTIONS.get(clean_name, f"Công cụ xử lý {clean_name}")
...
class Imagent:
    ...
    def run(self, plan: Optional[list[Subtask]] = None, cache: Optional[Path] = None) -> None:
        self.workflow_logger.info(f"Đang chạy Imagent với cấu hình: {self.profile_name}")
        self.workflow_logger.info(f"Phục hồi khuôn mặt: {self.face_restoration}")
        self.workflow_logger.info(f"Làm sáng ảnh: {self.brightening}")
        self.workflow_logger.info(f"Phục hồi ảnh cũ: {self.old_photo_restoration}")
        ...
        if plan is not None:
            ...
            self.workflow_logger.info(f"Kế hoạch: {self.plan}")
        else:
            self.propose()
    ...
    def propose(self) -> None:
        ...
        if self.restore_option:
            ...
            for task in explicit_tasks:
                ...
                if task == "super-resolution":
                    ...
                    self.workflow_logger.info(f"Hệ số phóng đại mục tiêu: {target_factor}")
                    if target_factor > 1:
        ...
        if hasattr(self, 'perception_agent'):
    ...
    def evaluate_degradation(self) -> list[tuple[Degradation, Level]]:
        ...
        self.workflow_logger.info(f"Đánh giá lỗi ảnh: {evaluation}")
        return evaluation
    ...
    def evaluate_degradation_by_gpt4v(self) -> list[tuple[Degradation, Level]]:
        def check_evaluation(evaluation: object):
            ...
            expected_degradations = {
                "noise", "motion blur", "defocus blur", "haze", "rain", "dark", "jpeg compression artifact"
            }
            assert rsp_degradations == expected_degradations, f"Invalid degradation: {rsp_degradations}. Expected: {expected_degradations}"
    ...
    def get_image_description(self) -> str:
        ...
        if self.evaluate_degradation_by == "gpt4v":
            # Use GPT-4V for image description
            prompt = "Please describe the content and style of this input image. The description must not contain its image quality."
            _, output = self.gpt4.query(
                img_path_lst=[Path(self.cur_node["img_path"])],
                prompt=prompt
            )
            self.workflow_logger.info(f"Image description: {output}")
            return output
    ...
    def schedule_w_retrieval(
        ...
        self.workflow_logger.info(f"Nhận định của AI: {schedule['thought']}")
    ...
    def tool_selection(self, subtask: Subtask, toolbox: list[Tool], max_side: int) -> list[Tool]:
        ...
        if "Fast" in self.profile_name and "super-resolution" in subtask and max_side >= self.fast4k_side_thres:
            self.workflow_logger.info("Chế độ nhanh: chỉ giữ lại các tool xử lý nhanh (cho super-resolution)")
    ...
    def execute_subtask(self, cache: Optional[Path]) -> bool:
        ...
        for tool in updated_toolbox:
            ...
            self.workflow_logger.info(f"Tool tốt nhất: {best_tool_name} ({get_tool_desc('tool-'+best_tool_name)})")
        ...
        if "super-resolution" in subtask:
            ...
            if output_max_side >= 1024:
                self.workflow_logger.info("Đang cân bằng lại màu sắc tổng thể...")
        ...
        self.workflow_logger.info(
            f"Kết quả {subtask.capitalize()}: "
            f"{self._img_nickname(self.cur_node['img_path'])} "
            f"với điểm chất lượng {best_img_score}.")
    ...
    def search_best_by_comp(self, candidates: list[Path]) -> Path:
        ...
        for i in range(1, len(candidates)):
            ...
            self.workflow_logger.info(
                f"Đang so sánh {self._img_nickname(best_img)} và {self._img_nickname(cur_img)}..."
            )
            ...
            if choice == "latter":
                ...
                self.workflow_logger.info(
                    f"Khó quyết định. Giữ lại {self._img_nickname(best_img)}."
                )
        self.workflow_logger.info(
            f"{self._img_nickname(best_img)} được chọn là tốt nhất."
        )
        return best_img
    ...
    def _prepare_for_subtask(
        ...
        self.workflow_logger.info(
            f"Đang thực hiện {subtask} trên {self._img_nickname(self.cur_node['img_path'])}..."
        )
    ...
    def _record_tool_res(self, img_path: Path) -> None:
        ...
        self.workflow_logger.info(
            f"{tool_name} ({get_tool_desc('tool-'+tool_name)}) được dùng để xử lý {degradation}, trình tự: {self._img_nickname(img_path)} ."
        )
    ...
    def _record_res(self) -> None:
        ...
        self.workflow_logger.info(
            f"Kết quả phục hồi cuối cùng: {self._img_nickname(self.res_path)}.")
