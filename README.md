...
<h1>Imagent: Agentic Any Image to 4K Super-Resolution</h1>
...
![visitors](https://visitor-badge.laobi.icu/badge?page_id=taco-group/Imagent)
...
- 🔥 **Profile Module**: To expand the applicability of **Imagent**, we propose a **Profile Module** to bring the availability to customize the system for different restoration tasks. **Imagent** can adapt to different restoration tasks without extra training.
...
<p align="center">
    <img src="./assets/Imagent_framework.png" width=95%>
<p>
...
**Prerequest:** Before running Imagent, please fill in the API key in [config file](config.yml)

The inference of Imagent relies on profile, we present examples here:
...
CUDA_VISIBLE_DEVICES=1 python infer_4kagent.py \
  ...
  --output_dir ./outputs/Imagent_test/classicsr \
...
CUDA_VISIBLE_DEVICES=1 python infer_4kagent.py \
  ...
  --output_dir ./outputs/Imagent_test/realworldsr \
...
# Imagent inference in portal B:
CUDA_VISIBLE_DEVICES=1 python infer_4kagent.py \
  --input_dir ./assets/profile_test_example/4ksr \
  --output_dir ./outputs/Imagent_test/4ksr \
...
CUDA_VISIBLE_DEVICES=1 python infer_4kagent.py \
  ...
  --output_dir ./outputs/Imagent_test/opr \
...
CUDA_VISIBLE_DEVICES=1 python infer_4kagent.py \
  ...
  --output_dir ./outputs/Imagent_test/mir \
...
Currently, Imagent will generate a folder which contains logs, images in inference. If we only need the final output image for calculating metrics (e.g., PSNR / SSIM / LPIPS / ...), we can use this script to extract every `output` image into a new folder with their original image name.

[2] Extract result toolchain: [utils/toolchain_export.py](utils/toolchain_export.py) 

If we run multiple images and we want to know the tool-chain of Imagent for each image, we can use this script to extract every tool-chain of each image. For example,
...
We evaluate Imagent on 11 different image SR tasks. The overall experiment results are summarized as follows:
...
@article{zuo20254kagent,
      title={Imagent: Agentic Any Image to 4K Super-Resolution}, 
